...
<body>
  ...
  <script>
    ...
    // Location finder
    let map = L.map('map').setView([26.8467, 80.9462], 13); // Default center (Lucknow)
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  attribution: '© OpenStreetMap contributors'
}).addTo(map);

let marker;
function showOnMap(lat, lng, bus){
  if(marker){
    marker.setLatLng([lat, lng]);
    marker.setPopupContent(`Bus ${bus}`).openPopup();
  } else {
    marker = L.marker([lat, lng]).addTo(map).bindPopup(`Bus ${bus}`).openPopup();
  }
      ...
      if(data){
        resultDiv.innerHTML = 
          `Bus ${bus} is at Lat: ${data.lat}, Lng: ${data.lng} <br>(Last update: ${data.time})`;
           showOnMap(data.lat, data.lng, bus);
      } else {
